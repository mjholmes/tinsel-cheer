...
// Original: https://gist.github.com/technobly/8339548
// Modified by Matt Holmes Dec 2015.
// https://github.com/mjholmes/tinsel-cheer
...
//MJH#include "ThingSpeak/ThingSpeak.h"
...
void setColour(uint8_t wait, String colour) {
  uint16_t pixel;
  int newR = 0x10, newG = 0x10, newB = 0x10;
  ...
  if(colour == "pink")      { newR = 0xff; newG = 0xb0; newB = 0xbb; }
  ...
  colorWipe(Color(newR, newG, newB), wait);
...
// Alternate (static) Red, Green, Amber, Blue lights
void xmas(uint16_t wait) {
      uint8_t j = 0;
      for(int8_t i=0; i<strip.numPixels(); i++) {
          switch(j){
            case 0:
              strip.setPixelColor(i, Wheel(85) ); // Red
              j++;
              break;
            case 1:
              strip.setPixelColor(i, Wheel(1) ); // Green
              j++;
              break;
            case 2:
              strip.setPixelColor(i, Wheel(60) ); // Amber
              j++;
              break;
            case 3:
              strip.setPixelColor(i, Wheel(180) ); // Blue
              j = 0;
              break;
          }
          strip.show();
